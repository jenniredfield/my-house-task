...
export function createListWithDates(
  housemates,
  tasks,
  startingDate,
  numbersOfWeeksElapsed
) {
  ...
  for (let i = lastWeek; i < nextFewWeeks; i++) {
    listOfTasks.push(
      calculateHouseMateWithTask(housemates, tasks, startingDate, i)
    );
  }
...
export function calculateHouseMateWithTask(
  housemates,
  tasks,
  startingDate,
  numbersOfWeeksElapsed
) {
  const weekStartDate = moment(startingDate, 'DD/MM/YYYY')
    .add(String(numbersOfWeeksElapsed), 'weeks')
    .format('DD/MM');
  const weekEndDate = moment(startingDate, 'DD/MM/YYYY')
    .add(String(numbersOfWeeksElapsed), 'weeks')
    .add('6', 'days')
    .format('DD/MM');
